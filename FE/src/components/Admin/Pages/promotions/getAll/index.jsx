...
function PromotionGetAll() {
    ...
    const getUsedPromotions = async () => {
        try {
            const res = await axios.get(`${Constants.DOMAIN_API}/api/promotions/applied`);
    ...
    const getPromotions = async (page = 1, search = "", status = "", start = "", end = "") => {
        try {
            ...
            const res = await axios.get(`${Constants.DOMAIN_API}/api/promotions/list`, { params });
    ...
    const confirmDelete = async (promo) => {
        const result = await Swal.fire({
            title: "Bạn có chắc muốn xóa?",
            text: `Khuyến mãi "${promo.name}" sẽ bị xóa nếu chưa được sử dụng.`,
            icon: "warning",
    ...
    const deletePromotion = async () => {
        ...
        try {
            await axios.delete(`${Constants.DOMAIN_API}/api/promotion/${selectedPromotion.id}`);
